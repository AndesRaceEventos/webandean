...
required List<Map<String, dynamic>> sourceOriginal
  ...
  List<TListaEntregaEquiposModel> datTable = listaProductos;
    List<TListaEntregaEquiposModel> dataSelected = selecteds.map((map){
      print('IMPRIMIR MAP: $map');
      final TListaEntregaEquiposModel producto = map['data'];
      return producto;
     }).toList();  
...
dynamic getValueFromModel(String key, TListaEntregaEquiposModel model) {
  switch (key) {
    ...
    case 'NRO.ITEMS':                    return model.listaEquipos?.length ?? 0;
