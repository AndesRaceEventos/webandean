...
import 'package:webandean/poketbase/entregas/poketbase_lista_equipos.dart';

import 'package:webandean/model/equipo/model_equipo.dart';
import 'package:webandean/model/entregas/model_lista_equipos.dart';

class TListaEquipoAppProvider with ChangeNotifier {

  String eventAction = '';
  String collectionName = TListaEquiposApp.collectionName;

  List<TListaEntregaEquiposModel> listProductos = [];

  TListaEquipoAppProvider() {
    print('$collectionName Inicializado');
    getProvider();
    realtime();
  }

  void addSet(TListaEntregaEquiposModel e) {
    listProductos.add(e);
    notifyListeners();
  }

  void updateSet(TListaEntregaEquiposModel e) {
    listProductos[listProductos.indexWhere((x) => x.id == e.id)] = e;
    notifyListeners();
  }

  void removeSet(TListaEntregaEquiposModel e) {
    listProductos.removeWhere((x) => x.id == e.id);
    notifyListeners();
  }

 Future<void> realtime() async {
  TListaEquiposApp.realTimeToPocketbase((event) {
    // Procesar la respuesta para convertirla en TProductosAppModel
    List<TListaEntregaEquiposModel> productos = TListaEquiposApp.processResponse([event.record!]);
...
bool hasUpdates = false; // Variable para rastrear si hubo actualizaciones
 ...
 Future<void> getProvider({bool isrethrow = false }) async {
  try {
    ...
    final options = TListaEquiposApp.getToPoketbase(filter: filter, expand: expand, sort: sort);
    ...
    List<TListaEntregaEquiposModel> productos = TListaEquiposApp.processResponse(response);
...
Future<void> refreshProvider() async {
  ...
  Future<void> saveProvider(
    ...
    notifyListeners();
     await ExceptionClass.tryCathCustom(
      task: () async {
         TListaEntregaEquiposModel data = TListaEntregaEquiposModel(
        id: id ?? '',
        qr: qr,
        nombre: nombre,
      ...
      // Determinar si es creación o actualización
       final isCreating = data.id == null || data.id.isEmpty;
          ...
          ? TListaEquiposApp.postToPoketbase(data: data, imagen: fileImagen, pdf: filePdf)
          : TListaEquiposApp.putToPoketbase(id: data.id, data: data, imagen: fileImagen, pdf: filePdf);
 ...
 Future<void> deleteTProductosApp({required String id, required BuildContext context}) async {
    ...
    notifyListeners();
     await ExceptionClass.tryCathCustom(
      task: () async {
        final operation = TListaEquiposApp.deleteToPoketbase(id: id);
  ...
  Map<String, List<TListaEntregaEquiposModel>> groupByDistance({required List<TListaEntregaEquiposModel> listData, required String fieldName}) {
    Map<String, List<TListaEntregaEquiposModel>> groupedData = {};
    String value;
  ...
  //Busqueda de datos
  List<TListaEntregaEquiposModel> filteredData = [];
  String searchText = '';

  void setSearchText(String text, List<TListaEntregaEquiposModel> listData) {
    if (text == "" || text == null) {
  ...
  bool filterByFields({required String value, required TListaEntregaEquiposModel data}) {
    List<String?> fields = [
      data.qr, 
      data.nombre, 
      data.observacion, 
  ...
  void clearSearch(List<TListaEntregaEquiposModel> listData) {
    searchText = '';
