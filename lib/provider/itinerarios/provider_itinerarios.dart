...
import 'package:webandean/poketbase/itinerarios/poketbase_itinerarios.dart';

import 'package:webandean/model/itinerarios/model_itinerario_programa_apu.dart';

class TItinerariosAppProvider with ChangeNotifier {

  String eventAction = '';
  String collectionName = TItinerariosApp.collectionName;

  List<TItinerioApuModel> listProductos = [];
  ...
  void addSet(TItinerioApuModel e) {
    listProductos.add(e);
    notifyListeners();
  }

  void updateSet(TItinerioApuModel e) {
    listProductos[listProductos.indexWhere((x) => x.id == e.id)] = e;
    notifyListeners();
  }

  void removeSet(TItinerioApuModel e) {
    listProductos.removeWhere((x) => x.id == e.id);
    notifyListeners();
  }

 Future<void> realtime() async {
  TItinerariosApp.realTimeToPocketbase((event) {
    // Procesar la respuesta para convertirla en TProductosAppModel
    List<TItinerioApuModel> productos = TItinerariosApp.processResponse([event.record!]);
...
bool hasUpdates = false; // Variable para rastrear si hubo actualizaciones
 ...
 Future<void> getProvider({bool isrethrow = false }) async {
  try {
    ...
    final options = TItinerariosApp.getToPoketbase(filter: filter, expand: expand, sort: sort);
    ...
    List<TItinerioApuModel> productos = TItinerariosApp.processResponse(response);
...
Future<void> refreshProvider() async {
  ...
  Future<void> saveProvider(
    ...
    required String? id,
    required String qr,
    required String tipoDesalida,
    required String nombre,
    required String itinerario,
    required int dias,
    required int noches,

    required int chillcaNight,
    required int machuNight,
    required int anantaNight,
    required int huampoNight,
    ...
    notifyListeners();
     await ExceptionClass.tryCathCustom(
      task: () async {
       TItinerioApuModel data = TItinerioApuModel(
        id: id ?? '',
        qr: qr,
        nombre: nombre,
        ...
        dias: dias,
        noches: noches,
        chillcaNight: chillcaNight,
        machuNight: machuNight,
        anantaNight: anantaNight,
        huampoNight: huampoNight,
      ...
      // Determinar si es creación o actualización
       final isCreating = data.id == null || data.id.isEmpty;
          ...
          ? TItinerariosApp.postToPoketbase(data: data, imagen: fileImagen, pdf: filePdf)
          : TItinerariosApp.putToPoketbase(id: data.id, data: data, imagen: fileImagen, pdf: filePdf);
  ...
  // Context
    required BuildContext context,
    //Coleciones 
    // List<TEquiposAppModel>? listaProducto,
    //IMAGES
    Uint8List? fileImagen,
    Uint8List? filePdf,
    //DOCUMENTOS
  required TItinerioApuModel data,
  }) async {
      isSyncing = true;
      notifyListeners();
 ...
 Future<void> deleteTProductosApp({required String id, required BuildContext context}) async {
    ...
    notifyListeners();
     await ExceptionClass.tryCathCustom(
      task: () async {
        final operation = TItinerariosApp.deleteToPoketbase(id: id);
  ...
  Map<String, List<TItinerioApuModel>> groupByDistance({required List<TItinerioApuModel> listData, required String fieldName}) {
    Map<String, List<TItinerioApuModel>> groupedData = {};
    String value;

    for (var e in listData) {
      switch (fieldName) {
        //string 
        case 'TIPO_SALIDA': value = ValidationField().getField(e.tipoDesalida); break;
        //bool 
  ...
  //Busqueda de datos
  List<TItinerioApuModel> filteredData = [];
  String searchText = '';

  void setSearchText(String text, List<TItinerioApuModel> listData) {
    if (text == "" || text == null) {
  ...
  bool filterByFields({required String value, required TItinerioApuModel data}) {
    List<String?> fields = [
      data.qr, 
      data.nombre, 
      data.observacion, 
      data.tipoDesalida.toString(), 
      // data.proveedor.join(', '), //list<String>
      ];
    return fields.any((field) => field.toString().toLowerCase().contains(value.toString().toLowerCase()));
  }

  void clearSearch(List<TItinerioApuModel> listData) {
    searchText = '';
