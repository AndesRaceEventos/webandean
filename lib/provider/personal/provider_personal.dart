...
import 'package:webandean/poketbase/personal/poketbase_personal.dart';

import 'package:webandean/model/personal/model_personal_apu.dart';

class TPersonalAppProvider with ChangeNotifier {

  String eventAction = '';
  String collectionName = TPersonalApp.collectionName;

  List<TPersonalApuModel> listProductos = [];
  ...
  void addSet(TPersonalApuModel e) {
    listProductos.add(e);
    notifyListeners();
  }

  void updateSet(TPersonalApuModel e) {
    listProductos[listProductos.indexWhere((x) => x.id == e.id)] = e;
    notifyListeners();
  }

  void removeSet(TPersonalApuModel e) {
    listProductos.removeWhere((x) => x.id == e.id);
    notifyListeners();
  }

 Future<void> realtime() async {
  TPersonalApp.realTimeToPocketbase((event) {
    // Procesar la respuesta para convertirla en TProductosAppModel
    List<TPersonalApuModel> productos = TPersonalApp.processResponse([event.record!]);
...
bool hasUpdates = false; // Variable para rastrear si hubo actualizaciones
 ...
 Future<void> getProvider({bool isrethrow = false }) async {
  try {
    ...
    final options = TPersonalApp.getToPoketbase(filter: filter, expand: expand, sort: sort);
    ...
    List<TPersonalApuModel> productos = TPersonalApp.processResponse(response);
...
Future<void> refreshProvider() async {
  ...
  Future<void> saveProvider(
    ...
    notifyListeners();
     await ExceptionClass.tryCathCustom(
      task: () async {
       TPersonalApuModel data = TPersonalApuModel(
        id: id ?? '',
        qr: qr,
        nombre: nombre,
      ...
      // Determinar si es creación o actualización
       final isCreating = data.id == null || data.id.isEmpty;
          ...
          ? TPersonalApp.postToPoketbase(data: data, imagen: fileImagen, pdf: filePdf)
          : TPersonalApp.putToPoketbase(id: data.id, data: data, imagen: fileImagen, pdf: filePdf);
  ...
  // Context
    ...
    Uint8List? filePdf,
    //DOCUMENTOS
  required TPersonalApuModel data,
  }) async {
      isSyncing = true;
      notifyListeners();
 ...
 Future<void> deleteTProductosApp({required String id, required BuildContext context}) async {
    ...
    notifyListeners();
     await ExceptionClass.tryCathCustom(
      task: () async {
        final operation = TPersonalApp.deleteToPoketbase(id: id);
  ...
  Map<String, List<TPersonalApuModel>> groupByDistance({required List<TPersonalApuModel> listData, required String fieldName}) {
    Map<String, List<TPersonalApuModel>> groupedData = {};
    String value;

    for (var e in listData) {
      switch (fieldName) {
        //string 
        case 'ROL': value = ValidationField().getField(e.idRol); break;
        //bool 
  ...
  //Busqueda de datos
  List<TPersonalApuModel> filteredData = [];
  String searchText = '';

  void setSearchText(String text, List<TPersonalApuModel> listData) {
    if (text == "" || text == null) {
  ...
  bool filterByFields({required String value, required TPersonalApuModel data}) {
    List<String?> fields = [
      data.qr, 
      data.nombre, 
      data.observacion, 
  ...
  void clearSearch(List<TPersonalApuModel> listData) {
    searchText = '';
